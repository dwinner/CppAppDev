/************************************************************************/
/* Enumerate some registry objects                                      */
/************************************************************************/

#include "pch.h"
#include <iostream>

BOOL TraverseRegistry(HKEY, LPCTSTR, LPCTSTR, LPBOOL);

BOOL DisplayPair(LPCTSTR, DWORD, LPBYTE, DWORD, LPBOOL);

BOOL DisplaySubKey(LPCTSTR, LPCTSTR, PFILETIME, LPBOOL);
...
int main(int argc, LPCTSTR argv[])
{
   ...
   for (i = 0;
        PreDefKeyNames[i] != nullptr && _tcscmp(PreDefKeyNames[i], keyName) != 0;
        i++);
   if (PreDefKeyNames[i] == nullptr)
      ReportError(_T("Use a Predefined Key"), 1, FALSE);
   hKey = PreDefKeys[i];
   ...
   ok = TraverseRegistry(hKey, argv[keyIndex], nullptr, flags);
   RegCloseKey(hKey);

   return ok ? 0 : 1;
}

BOOL TraverseRegistry(HKEY hKey, LPCTSTR fullKeyName, LPCTSTR subKey, LPBOOL flags)
...

{
   ...
   if (RegQueryInfoKey(hSubKey, nullptr, nullptr, nullptr,
   ...
   subKeyName = static_cast<LPWSTR>(malloc(TSIZE * (maxSubKeyLen + 1))); /* size in bytes */
   valueName = static_cast<LPWSTR>(malloc(TSIZE * (maxValueNameLen + 1)));
   value = static_cast<LPBYTE>(malloc(maxValueLen)); /* size in bytes */
   ...
   for (index = 0; index < numSubKeys; index++)
   {
      ...
      if (GetLastError() == 0)
      {
         ...
         if (recursive)
         {            
            swprintf_s(fullSubKeyName, MAX_PATH+1, _T("%s\\%s"), fullKeyName, subKeyName);
...

{
   ...
   case REG_DWORD: /* 4: A 32-bit number. */
      _tprintf(_T("%x"), static_cast<DWORD>(*value));
      break;

   case REG_EXPAND_SZ:
      /* 2: null-terminated string with unexpanded references to environment variables (for example, “%PATH%”). */
