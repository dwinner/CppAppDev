...
std::unique_ptr<std::valarray<std::unique_ptr<Nalu>>> H264FileReader::GetNalUnits() const
{
    ...
    vector<string> splitByAudList = SplitByRegex(h264Content);
...
std::vector<std::string> H264FileReader::SplitByRegex(const std::string &inputString)
{
    using namespace std;

    sregex_token_iterator start{inputString.begin(), inputString.end(), AudNaluRegEx, {-1, 0}}, end;
