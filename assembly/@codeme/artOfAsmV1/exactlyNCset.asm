...
; that are members of a character set. The
; n+1st character either must not exist or
; must not be a member of the character set.
;
...
nLoop:      inc     rsi
            ...
            cmp     cl, 0           ;End of str; matched less
            je      failed          ; than n chars, so fail
            mov     ebx, ecx
            and     ecx, 7
            shr     ebx, 3
            mov     eax, 1
...
; When we match the first n charcters above, we must
; check to make sure we're at the end of the string
; or that the next character is not a member of the set.
