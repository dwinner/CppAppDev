...
namespace manifest_client
{
   std::vector<CString> read_manifest(const char* sAddr, u_short udpPort, u_short tcpPort)
   {
      ...
      throw_if_error(fd, "Init udp socket");
      ...
      throw_if_error(bindResult, "Calling bind failed");
      ...
      throw_if_error(setOptResult, "Call setsockopt failed");
      ...
      throw_if_error(returnValue, "Calling select failed");
      if (returnValue == 0)
      {
      ...
      throw_if_error(fd, "Init tcp socket failed");
      from.sin6_port = htons(tcpPort);
      throw_if_error(connect(fd, reinterpret_cast<sockaddr*>(&from), fromlen), "Calling connect failed");
      ...
      throw_if_error(sendResult, "Calling send failed");

      // shutdown the connection since no more data will be sent
      throw_if_error(shutdown(fd, SD_SEND), "Calling shutdown failed");

      rxLen = 0;
      do
      {
         rxLen = recv(fd, reinterpret_cast<char*>(rxData), sizeof rxData, 0);
         throw_if_error(rxLen, "Calling tcp recv failed");
   ...
   void throw_if_error(const int ret, const char* sErr)
   {
      if (ret < 0)
      {
