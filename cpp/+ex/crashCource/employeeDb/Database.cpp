...
namespace HR
{
   Employee &Database::addEmployee(const std::string &firstName, const std::string &lastName, EmpType empType)
   {
      Employee empToAdd{firstName, lastName, empType};
      empToAdd.setEmployeeNumber(_nextEmployeeNumber++);
      empToAdd.hire();
      _employees.emplace_back(empToAdd);
