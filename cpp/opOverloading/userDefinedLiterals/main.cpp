...
public:
   ...
   friend Length operator ""_km(long double kilometerVal);

   friend Length operator ""_m(long double meterVal);
...
Length operator ""_km(long double kilometerVal)
{
   return Length{kilometerVal * 1000}; // Convert to meters.
}

// Cooked _m literal operator
Length operator ""_m(long double meterVal)
{
   return Length{meterVal};
}
