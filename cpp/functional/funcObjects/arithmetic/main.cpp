...
template<input_iterator Iter, copy_constructible StartValue, invocable<const StartValue &, const StartValue &> Operation>
...
{
   auto mult{
       accumulateData(cbegin(values), cend(values), 1, multiplies<int>{})
   };
...
{
   auto mult{
       accumulateData(cbegin(values), cend(values), 1, multiplies<>{})
   };
...
int main()
{
   ...
   println("The geometric mean is {}",
           geometricMean(myVector));
...

   {
      ...
      double result{
          accumulateData(cbegin(values), cend(values), 1.1, multiplies<>{})
      };
      println("{}", result);
   }
