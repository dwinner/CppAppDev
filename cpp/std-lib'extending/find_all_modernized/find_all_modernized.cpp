...
OutputIterator FindAll(
   ForwardIterator first, Sentinel last, OutputIterator dest, Predicate pred, Projection proj = {})
{
   while (first != last)
   {
...
int main()
{
   ...
   FindAll(begin(vec), end(vec), back_inserter(matches),
           [](int i)
           {
              return i == 10;
           },
